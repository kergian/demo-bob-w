...
const useDateTime = (startDate?: string, endDate?: string) => {
  ...
  const checkDateOverlaps = (bookingData: APIBooking, roomId?: number) => {
    ...
    if (roomEndToUserStart) {
      ...
      !isEndsBefore && (shouldDisplay = false)
      !isEndsBefore && console.log('Room end overlaps with user booking start', roomId)
    }
    ...
    if (userEndToRoomStart) {
      ...
      !isStartsAfter && (shouldDisplay = false)
      !isStartsAfter && console.log('User booking end overlaps with room booking start', roomId)
    }
    ...
    if (!roomEndToUserStart && !userEndToRoomStart) {
      ...
      overlaps !== -1 && (shouldDisplay = false)
      overlaps !== -1 && console.log('Found other overlapping dates', roomId)
    }
